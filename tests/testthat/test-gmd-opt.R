...
test_that("1.3 — ensemble value unchanged after sort upgrade (G4)", {
  ...
  # aggregation is serial post-loop. Within one platform, output is
  # bit-deterministic. Cross-platform FP accumulates over n_boot=500 bootstraps:
  # bound is n_boot * machine_eps ~ 1.1e-13; observed max ~2.5e-14.
  set.seed(42)
  v_after <- robscale:::cpp_scale_ensemble(rnorm(200L), 500L)
  v_pin   <- readRDS(testthat::test_path("fixtures/ens_baseline_pin.rds"))
  expect_equal(v_after, v_pin, tolerance = 500L * .Machine$double.eps,
