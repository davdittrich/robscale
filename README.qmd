...
# M-estimators
loc_data <- m_analyzed %>% filter(expr == "robLoc") %>% 
  summarize(med = min(median_speedup), lo = min(ci_low), hi = max(ci_high))
scale_data <- m_analyzed %>% filter(expr == "robScale") %>% 
  summarize(med = min(median_speedup), lo = min(ci_low), hi = max(ci_high))
adm_data <- m_analyzed %>% filter(expr == "adm") %>% 
  summarize(med = min(median_speedup), lo = min(ci_low), hi = max(ci_high))
...
api_data <- data.frame(
  ...
  `Speedup vs revss` = c("—", "—", 
                         format_speedup_ci(loc_data$med, loc_data$lo, loc_data$hi),
                         format_speedup_ci(scale_data$med, scale_data$lo, scale_data$hi),
                         format_speedup_ci(adm_data$med, adm_data$lo, adm_data$hi)),
  `Speedup vs robustbase` = c(
    format_speedup_ci(min(qn_analyzed$median_speedup), min(qn_analyzed$ci_low), max(qn_analyzed$ci_high)),
    format_speedup_ci(min(sn_analyzed$median_speedup), min(sn_analyzed$ci_low), max(sn_analyzed$ci_high)),
    "—", "—", "—"),
  check.names = FALSE
)
