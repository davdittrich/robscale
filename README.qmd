...
an $O(n \log n)$ operation, with sorting networks applied for $n \le 16$.
...
to zero (i.e. MAD is $\leq$ `implbound`), the `fallback` argument
controls the result:
...
median-distance-to-zero, exactly $1.4826 \cdot \text{median}(|x_i - \mu|)$, as the
...
sample size---useful for small-sample bias for $n$ observations.
...
(the logistic CDF), which is $1/(1 + e^{-x})$. The computation requires one call to
...
single linear scan over the upper partition locates element $k + 1$
needed for averaging.

For $n \leq 16$---the core target regime---the selection step uses optimal
sorting networks (Knuth, TAOCP Vol. 3, Sec. 5.3.4, and Dobbelaere's verified
optimal networks for $n = 9$ to $n = 16$). These are conditional compare-and-swap
...
Cross-platform benchmarking confirmed speedups of 2 to 4 $\times$ over `std::sort`
