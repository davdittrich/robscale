...
completes in `r qn_8_robscale` µs vs. `r qn_8_robustbase` µs for `robustbase` — a **`r qn_8_speedup`×** edge.

**Large samples ($n \ge 10^4$).** The advantage grows to **`r large_speedup_min`–`r large_speedup_max`×** as TBB
parallelism engages.
 At $n = 10^7$, `qn` runs in `r qn_10m_robscale` s vs. `r qn_10m_robustbase` s for
`robustbase::Qn` (**`r qn_10m_speedup`×**), and `sn` runs in `r sn_10m_robscale` s vs. `r sn_10m_robustbase` s (**`r sn_10m_speedup`×**).
...
```{r}
#| label: fig-fast-slow
#| fig-cap: "Median speedup factor (×) of optimized vs. unoptimized builds with 95% BCa confidence bands. Note logarithmic scales."
#| fig-width: 8
#| fig-height: 5
tar_read(fast_slow_figure)
```
