...
uses dual $O(n)$ Floyd--Rivest selection, which dominates at large $n$.

**MAD** (`robscale::mad_scaled` vs `stats::mad`): **`r mad_stats_min`--`r mad_stats_max`x** speedup.
`stats::mad` also performs a full sort for the median step. `robscale` uses
$O(n)$ selection throughout.
