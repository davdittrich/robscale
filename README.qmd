...
M-estimation and `robustbase` for the $Q\_n$ and $S\_n$ scale estimators---carry
...
iteration; $Q\_n$ and $S\_n$ use parallelized $O(n \log n)$ algorithms via TBB.
...
**`r sn_speedup_min`--`r sn_speedup_max`x** for $S\_n$ and
**`r qn_speedup_min`--`r qn_speedup_max`x** for $Q\_n$---with gains peaking near
...
this identity to reduce $\psi\_{\log}$ to a single `tanh` call. The identity is not merely a cosmetic rewrite:
...
derivative of the logistic psi satisfies $\psi\_{\log}'(x) = 1 - \psi\_{\log}^2(x) = 1 - \tanh^2(x/2)$.
...
median of the vector $\{\text{med}\_j |x\_i - x\_j|\}\_{i=1}^n$. `robscale`
...
each bootstrap replicate $r = 1, \ldots, n\_{\text{boot}}$:
...
replicates. The final estimate is $\hat\sigma = \sum\_j w\_j \cdot \hat\sigma\_j(x)$
...
**$Q\_n$ and $S\_n$ statistics.** $Q\_n = c\_n \cdot d \cdot \{|x\_i - x\_j|; i < j\}\_{(k)}$
where $k = \binom{h}{2}$, $h = \lfloor n/2 \rfloor + 1$, and $d = 2.2191$
(consistency constant for Gaussian data). $S\_n = c\_n' \cdot 1.1926 \cdot
\text{lomed}\_i \{\text{himed}\_j |x\_i - x\_j|\}$, where $\text{lomed}$ and
...
$\frac{1}{\binom{n}{2}}\sum\_{i<j}|x\_i - x\_j|$ but avoids materializing
$O(n^2)$ pairs.
