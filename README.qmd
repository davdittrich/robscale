...
algorithm for small $n$ (below `qn_exact_threshold`) and a parallelized
Johnson-style algorithm for larger samples, with parallelism thresholds
derived from the detected per-core L2 cache size at runtime.
...
qn_table_data <- qn_analyzed %>%
  filter(n %in% c(8, 64, 1024, 65536, 10000000)) %>%
  rowwise() %>%
  mutate(
...
> (including SIMD flags like `-mavx2 -mfma`), install the package from source
> with the `ROBSCALE_FAST=1` environment variable set:
>
> **From the terminal:**
> ```bash
...
> compatibility across all architectures. Cache-aware parallelism thresholds
> are derived from the detected L2 cache size at runtime in both builds.
