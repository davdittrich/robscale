...
M-estimators and **2.6--4.1x** for `robLoc()` at $n = 100$ to $10{,}000$
via a fused single-pass AVX2 Newton--Raphson kernel. Against `robustbase`, it achieves
...
- Fusing the three-pass Newton--Raphson inner loop into a single AVX2 pass for `robLoc()` (v0.4.0).
...
- Passing the hot copy buffer to `adm_core()`, reducing a redundant cache miss per call (v0.4.0).
