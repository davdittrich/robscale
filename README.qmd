...
bootstrap for `scale_robust()`). The exceptions are `adm()` and `robLoc()`,
which do not support confidence intervals.
...
Rousseeuw & Croux (1993). While it maintains the same core algorithmic
foundations, the newest version of `revss` uses updated bias correction factors,
meaning results may not be numerically identical. `robscale` focus remains
on computational efficiency, where it continues to cut run times significantly
