...
All estimators are implemented as C++17 kernels with vectorized tanh evaluation
where batch sizes justify the overhead, Newton--Raphson iteration, and parallelized
...
sample size---useful for small-$n$ bias correction.
...
`iqr_scaled` and `mad_scaled` use a separate adaptive path (`adaptive_median_select`
in `src/pdq_select.h`): Floyd-Rivest for $n \leq \max(2048, L_{2,\text{per-core}} /
(\texttt{sizeof(double)} \times 5))$, and pdqselect (miniselect) above that
threshold, where pattern-defeating selection reduces constant factors on structured
inputs.
...
$O(n)$ selections---one per quartile---using the `adaptive_median_select`
strategy from `src/pdq_select.h`. This two-tier dispatch chooses the algorithm
based on a runtime threshold derived from the per-core L2 cache size:

- **Floyd-Rivest** (`median_select`): used when $n \leq$ `pdq_median_threshold`.
  Introselect guarantees $O(n)$ worst-case selection with median-of-three pivot
  selection.
- **pdqselect** (`pdq_median_select`): used when $n >$ `pdq_median_threshold`,
  where the pattern-defeating properties of pdqselect (from the miniselect
  library) reduce constant-factor overhead on structured inputs.

The threshold is computed at runtime as
$\max(2048,\; L_{2,\text{per-core}} / (\texttt{sizeof(double)} \times 5))$,
so it adapts to the host machine's cache hierarchy without requiring recompilation.

Each selection call uses the Type 7 quantile interpolation (the R default),
locating the floor index and then finding the next-smallest element via a linear
scan for the fractional interpolation. This gives $O(n)$ total complexity vs
the $O(n \log n)$ full sort used by `stats::IQR`. The MAD estimator uses the
same `adaptive_median_select` path for its two median computations.
...
graph TD
    ...
    subgraph "Algorithm Tiers"
        ...
        T4["IQR/MAD: adaptive_median_select\n(Floyd-Rivest <= pdq_threshold, pdqselect above)"]
    end
    ...
    subgraph "Hardware Acceleration"
        G[AVX2 / NEON (AVX512 with -mavx512f)]
        H[Apple Accelerate]
        I[SLEEF Library]
    end
