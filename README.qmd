...
m_fast <- benchmark_report$fast$m_estimators
m_legacy <- benchmark_report$legacy$revss

m_combined <- bind_rows(
  m_fast %>% mutate(label = "fast"),
  m_legacy %>% mutate(label = "legacy")
) %>%
  mutate(expr = as.character(expression)) %>%
  select(n, expr, median, label) %>%
  pivot_wider(names_from = label, values_from = median) %>%
  mutate(speedup = as.numeric(legacy) / as.numeric(fast))

m_speedup_min <- format_speedup(min(m_combined$speedup))
m_speedup_max <- format_speedup(max(m_combined$speedup))

# 2. Processing Scale-estimators (Panel B: robscale (fast) vs robustbase)
s_fast <- benchmark_report$fast$scale_estimators
s_legacy <- benchmark_report$legacy$robustbase

s_combined <- bind_rows(
  s_fast %>% mutate(label = "fast"),
  s_legacy %>% mutate(label = "legacy")
) %>%
  mutate(expr = as.character(expression)) %>%
  select(n, expr, median, label) %>%
  pivot_wider(names_from = label, values_from = median) %>%
...
qn_combined <- s_combined %>% filter(expr == "qn")
sn_combined <- s_combined %>% filter(expr == "sn")

qn_speedup_min <- format_speedup(min(qn_combined$speedup))
qn_speedup_max <- format_speedup(max(qn_combined$speedup))

sn_speedup_min <- format_speedup(min(sn_combined$speedup))
sn_speedup_max <- format_speedup(max(sn_combined$speedup))

# 3. Processing Optimized vs Unoptimized (Variation: fast vs slow)
# We aggregate all fast vs all slow
v_fast <- bind_rows(m_fast, s_fast)
v_slow <- bind_rows(benchmark_report$slow$m_estimators, benchmark_report$slow$scale_estimators)

v_combined <- bind_rows(
  v_fast %>% mutate(label = "fast"),
  v_slow %>% mutate(label = "slow")
) %>%
  mutate(expr = as.character(expression)) %>%
  select(n, expr, median, label) %>%
  pivot_wider(names_from = label, values_from = median) %>%
  mutate(speedup = as.numeric(slow) / as.numeric(fast))
```
...
achieves **`r qn_speedup_min`–`r qn_speedup_max`×** for $S_n$ and **`r qn_speedup_min`–`r qn_speedup_max`×** for $Q_n$ —with gains peaking near
...
# M-estimators
loc_speedup <- m_combined %>% filter(expr == "robLoc") %>% 
  summarize(min = min(speedup), max = max(speedup))
scale_speedup <- m_combined %>% filter(expr == "robScale") %>% 
  summarize(min = min(speedup), max = max(speedup))
adm_speedup <- m_combined %>% filter(expr == "adm") %>% 
  summarize(min = min(speedup), max = max(speedup))
...
api_data <- data.frame(
  ...
  `Speedup vs revss` = c("—", "—", 
                         sprintf("**%s – %s**", format_speedup(loc_speedup$min), format_speedup(loc_speedup$max)),
                         sprintf("**%s – %s**", format_speedup(scale_speedup$min), format_speedup(scale_speedup$max)),
                         sprintf("**%s – %s**", format_speedup(adm_speedup$min), format_speedup(adm_speedup$max))),
  `Speedup vs robustbase` = c(sprintf("**%s – %s**", qn_speedup_min, qn_speedup_max),
                            sprintf("**%s – %s**", sn_speedup_min, sn_speedup_max),
                            "—", "—", "—"),
  check.names = FALSE
)
...
The following table explicitly compares the performance of `robscale` when compiled with peak optimizations (`ROBSCALE_FAST=1`) against the unoptimized (portable) version of the package.

```{r}
#| label: tbl-comp-fast
#| results: asis

comp_data <- v_combined %>%
  group_by(expr) %>%
  summarize(
    `Min Speedup` = format_speedup(min(speedup)),
    `Max Speedup` = format_speedup(max(speedup))
  ) %>%
  rename(Estimator = expr)

knitr::kable(comp_data)
```

The gains in the optimized build come primarily from SIMD vectorization (AVX2/AVX512/Accelerate) and aggressive transcendental function expansion. On large samples, the optimized version also benefits from tuned threading thresholds that more effectively leverage TBB.
