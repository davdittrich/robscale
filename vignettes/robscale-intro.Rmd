...
2. **glibc libmvec** (`_ZGVdN4v_tanh`): AVX2-vectorized `tanh` on Linux
   x86\_64 with glibc $\geq$ 2.22 (Ubuntu 16.04+, Arch, Fedora 23+).
3. **SLEEF**: AVX2-vectorized fallback when libmvec is absent at link time.
4. **OpenMP SIMD** (`#pragma omp simd`): compiler-guided portable fallback.
5. **Scalar** `std::tanh`: universal fallback.

For $n < 8$, vectorized paths are bypassed.
...
**`robScale`: Aitken $\Delta^2$ acceleration.** The multiplicative fixed-point
iteration for scale (shown in the flowchart above) converges linearly.
`robScale` applies Aitken's $\Delta^2$ process (Aitken 1926; Steffensen 1933)
in Steffensen mode to accelerate it: after every two standard steps that produce
the triplet $(s_0, s_1, s_2)$, the algorithm extrapolates to

$$
s_{\text{acc}} = s_2 - \frac{(s_2 - s_1)^2}{s_2 - 2\,s_1 + s_0}
$$

This eliminates the dominant linear error term and converts the pair of steps
into an effectively superlinear update. Two acceptance conditions protect the
extrapolant: the sequence must be monotone and contracting
($d_1 d_2 > 0$ and $|d_2| < |d_1|$ where $d_k = s_k - s_{k-1}$), and the
candidate must be positive. When the sequence oscillates ($d_1 d_2 < 0$),
$s_1$ and $s_2$ bracket $s^*$; the algorithm falls back to the geometric mean
$\sqrt{s_1 s_2}$, which halves the log-distance to $s^*$ per outer iteration.
Across typical data, Aitken acceleration reduces the iteration count by
30--50\% at small $n$ and by approximately 20\% at large $n$.
...
**16**, 64--72. <https://doi.org/10.1080/03461238.1933.10419209>
